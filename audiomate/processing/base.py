...
class Processor(metaclass=abc.ABCMeta):
    ...
    data on different levels (Corpus, Utterance, File).
    For every level there is an offline and an online method.
    In the offline mode the data is processed in one step
    (e.g. the whole file/utterance at once). This means the
    ``process_frames`` method is called with all the frames
    of the file/utterance. In online mode the data is processed in chunks,
    so the ``process_frames`` method is called multiple times
    per file/utterance with different chunks.

    To implement a concrete processor the ``process_frames`` method
    has to be implemented. This method is called in online and offline mode.
    So it is up to the user to determine if a processor can be called in
    either online or offline mode, maybe both. This differs between use cases.
    ...
    Frame-size and hop-size are measured in samples regarding
    the original audio signal (or simply its sampling rate).
    """
    ...
    def process_corpus(self, corpus,
                       ...
                       sr=None):
        """
        Process all utterances of the given corpus and save
        the processed features in a feature-container. The utterances are
        processed in **offline** mode so the full utterance in one go.
        ...
        Args:
            ...
            sr (int): Use the given sampling rate. If ``None`` uses
                      the native sampling rate from the underlying data.

        Returns:
            FeatureContainer: Feature-container with the processed features.
        """

        def processing_func(utterance,
                            feat_container,
                            frame_size,
                            hop_size,
                            sr,
                            corpus):
            data = self.process_utterance(utterance,
        ...
        return self._process_corpus(corpus,
                                    output_path,
                                    processing_func,
                                    frame_size=frame_size,
                                    hop_size=hop_size,
    ...
    def process_corpus_online(self, corpus,
                              output_path,
                              frame_size=400,
                              hop_size=160,
                              sr=None,
                              ...
                              buffer_size=5760000):
        """
        Process all utterances of the given corpus and
        save the processed features in a feature-container.
        ...
        Args:
            ...
            buffer_size (int): Number of samples to load into memory at once.
                               The exact number of loaded samples depends on
                               the block-size of the audioread library.
                               So it can be of block-size higher, where the
                               block-size is typically 1024 or 4096.
    ...
    def process_features(self, corpus, input_features, output_path):
        """
        Process all features of the given corpus and save
        the processed features in a feature-container. The features are
        processed in **offline** mode, all features of an utterance at once.

        Args:
            corpus (Corpus): The corpus to process the utterances from.
            input_features (FeatureContainer): Features to process.
            output_path (str): A path to save the feature-container to.
        ...
        tf_frame, tf_hop = self.frame_transform(input_features.frame_size,
                                                input_features.hop_size)
        feat_container.frame_size = tf_frame
        feat_container.hop_size = tf_hop
    ...
    def process_features_online(self, corpus,
                                input_features,
                                output_path,
                                chunk_size=1):
        """
        Process all features of the given corpus and save
        the processed features in a feature-container.
    ...
    def process_utterance(self, utterance,
        ...
        Args:
            ...
            corpus (Corpus): The corpus the utterance is part of, if available.

        Returns:
            np.ndarray: The processed features.
        """
        return self.process_file(utterance.file.path,
                                 frame_size=frame_size,
                                 hop_size=hop_size,
                                 sr=sr,
    ...
    def process_utterance_online(self, utterance,
                                 frame_size=400,
                                 hop_size=160,
                                 sr=None,
                                 chunk_size=1,
        ...
        Args:
            ...
            buffer_size (int): Number of samples to load into memory at once.
                             The exact number of loaded samples depends on
                             the block-size of the audioread library.
                             So it can be of block-size higher,
        ...
        """
        return self.process_file_online(utterance.file.path,
                                        frame_size=frame_size,
                                        hop_size=hop_size,
                                        sr=sr,
                                        start=utterance.start,
    ...
    def process_file(self, file_path,
        ...
        Args:
            ...
            sr (int): Use the given sampling rate. If ``None``,
                      uses the native sampling rate from the underlying data.
            start (float): Num. seconds from file-start, to start processing.
            end (float): Num. seconds from file-start, to end processing.
            utterance (Utterance): The utterance that is associated
                                   with this file, if available.
        ...
        if num_pad_samples > samples.size:
            pad_width = (0, num_pad_samples - samples.size)
        ...
        frames = librosa.util.frame(samples, frame_length=frame_size,
    ...
    def process_file_online(self, file_path,
                            frame_size=400,
                            hop_size=160,
                            sr=None,
                            start=0,
        ...
        Args:
            ...
            buffer_size (int): Number of samples to load into memory at once.
                             The exact number of loaded samples depends on
                             the block-size of the audioread library.
                             So it can be of block-size higher, where
                             the block-size is typically 1024 or 4096.
        ...
        for frame, out_sr, last in audio.read_frames(file_path,
                                                     frame_size,
                                                     hop_size,
                                                     sr_target=sr,
                                                     start=start,
    ...
    def process_frames(self, data,
        ...
        Args:
            data (np.ndarray): Array of frames (num-frames x frame-dimensions).
            sampling_rate (int): The sampling rate of the underlying signal.
            offset (int): The index of the first frame in the chunk.
                          In offline mode always 0.
                          (Relative to the first frame of the utt/sequence)
            last (bool): ``True`` indicates that this is
                         the last frame of the sequence/utterance.
                         In offline mode always True.
            utterance (Utterance): Utterance the frame is from, if available.
    ...
    def frame_transform(self, frame_size, hop_size):
        ...
        feature-container. In the end one can calculate start and end time
        of a frame with this information.

        By default it is assumed that the processor doesn't change
        the frame-size and the hop-size.
    ...
    def _process_corpus(self, corpus,
                        ...
                        sr=None):
        """
        Utility function for processing a corpus
        with a separate processing function.
        """
        feat_container = assets.FeatureContainer(output_path)
        feat_container.open()
        ...
        for utterance in corpus.utterances.values():
            ...
            processing_func(utterance, feat_container,
                            frame_size, hop_size, sr, corpus)
